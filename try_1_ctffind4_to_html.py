...
while True:
	...
	if os.path.isfile('avg_chunk_10.json'):
		...
		with open(json_to_open, 'r') as f:
			...
			if len(avg_defocus_glob_2f) > 10 and len(avg_defocus_glob_2f)%10 == 0:
				j = int(len(avg_defocus_glob_2f)/10)
				for i in range(0,j):
					avg_chunk_of_10.append(avg_defocus_glob_2f[i*10:i*10+10])
	else:
		avg_chunk_of_10 = []
		if len(avg_defocus_glob_2f) > 10 and len(avg_defocus_glob_2f)%10 == 0:
			j = int(len(avg_defocus_glob_2f)/10)
	...
	if len(avg_chunk_of_10) >= 1:
		for i in range(0,len(avg_chunk_of_10)):
			ax[1,1].scatter(range(i*10, i*10+10), avg_chunk_of_10[i], s=2)
